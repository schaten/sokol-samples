...
//
float sampleShadow(sampler2D sm, vec2 uv, float compare) {
    #if !SOKOL_GLSL
    uv.y = 1.0-uv.y;
    #endif
    float depth = decodeDepth(texture(sm, vec2(uv.x, uv.y)));
    depth += 0.001;
...
//
float sampleShadowPCF(sampler2D sm, vec2 uv, vec2 smSize, float compare) {
    float result = 0.0;
    for (int x=-2; x<=2; x++) {
        for (int y=-2; y<=2; y++) {
            vec2 off = vec2(x,y)/smSize;
            result += sampleShadow(sm, uv+off, compare);
        }
    }
    return result / 25.0;
}
