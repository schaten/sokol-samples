...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    (void)argv;
    sapp_desc desc = { };
    desc.window.init_cb = init;
    desc.window.frame_cb = frame;
    desc.window.event_cb = event;
    desc.window.cleanup_cb = cleanup;
    desc.window.width = 832;
    desc.window.height = 600;
    desc.window.title = "Events";
    desc.window.user_cursor = true;
    desc.window.enable_clipboard = true;
    desc.window.enable_dragndrop = true;
    desc.window.max_dropped_files = max_dropped_files;
    desc.icon.sokol_default = true;
    desc.gl.force_gles2 = true;
    return desc;
}
